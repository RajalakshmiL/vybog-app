...
import * as StyledDOM from "../../Candidate-Page/ImportCandidate/CandidateFieldMapping.jsx";

function ClientFieldMapping() {
  const navigate = useNavigate();
  const { staticdata = {} } = jsonData || {};
  const { clientFieldMappings = {} } = staticdata || {};
  const { headerData = [], sampleData = [] } = clientFieldMappings || {};

  const { staticData = {} } = OptionData || {};
  const { ClientMappingsOptions = [] } = staticData || {};
  ...
  const getAvailableOptions = (currentKey) => {
    ...
    return ClientMappingsOptions.filter(
      (option) =>
  ...
  return (
    <StyledDOM.StyledSplitScreen className="split-container">
      ...
      >
        <StyledDOM.StyledHeading>Client Field Mapping</StyledDOM.StyledHeading>
...
export default ClientFieldMapping;
